...
class VLMService:
    ...
    async def assemble_steps_from_segments(
        ...
        prompt = (
            ...
            "- ng_criteria 只写真正的缺陷（拿错、放错、掉落、损坏），不要把「尚未开始 / 仍在等待」写成 NG\n"
            "- timeout_seconds 仅作节拍参考，默认 0 表示不按时间停线\n"
    ...
    async def refine_steps(
        ...
        prompt = (
            ...
            f"整体概览：{overview}\n\n"
            "请优化这些步骤：\n"
            "1. 补充缺失的判定标准 ok_criteria 和 ng_criteria；ng_criteria 只写真正的缺陷，不要写「尚未开始」\n"
            "2. timeout_seconds 仅作节拍参考，默认 0（不按时间停线）\n"
